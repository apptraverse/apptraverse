...
wchar_t const kNotifyWindowClass[] = L"AppTraverseMessengerNotify";
wchar_t const kLoadingWindowClass[] = L"AppTraverseMessengerLoading";
...
void WinApp::OnIncrementalPublished() {
  ...
  session_.cv.notify_all();
  ApplyMessengerStructural(bytes, *ui_application_, ui_storage_, notify_,
                           &*model_proxy_);
}
...
int WinApp::Run(std::filesystem::path const& state_dir) {
  ...
  model_thread_ = std::thread([this, notify, done_event] {
    session_.Run([notify](MessengerPublicationKind kind) {
      UINT const message = kind == MessengerPublicationKind::Initial
  ...
  ui_domain_.reset();
  model_proxy_.reset();
  UnregisterMessengerWin32Classes();
