...
bool NeedsParticipantList(RoomControlType type) {
  return type == RoomControlType::kMembershipPrepare ||
         type == RoomControlType::kMembershipSnapshot;
}
...
std::vector<std::uint8_t> EncodeRoomControl(RoomControlMessage const& msg) {
  ...
  if (msg.type == RoomControlType::kMembershipReject) {
  ...
  // Prepared / Applied / Activate / Activated: revision only.
  return out;
}

std::optional<RoomControlMessage> TryDecodeRoomControl(std::uint8_t const* data,
                                                       std::size_t size) {
  if (!IsRoomControlPayload(data, size) || size < 4 + 1 + 1 + 8) {
    return std::nullopt;
  }
  auto const* p = data + 4;
